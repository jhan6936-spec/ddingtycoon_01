...
/** YYYY-MM-DD → 해당일 03:00 KST ISO (잘못된 값이면 오늘) */
function recordedAtISOFromDateString(dateStr) {
  const match = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(dateStr || '').trim())
  if (!match) return getSeoulDailyRecordedAtISO()
  const year = Number(match[1])
  const month = Number(match[2])
  const day = Number(match[3])
  if (!year || month < 1 || month > 12 || day < 1 || day > 31) {
    return getSeoulDailyRecordedAtISO()
  }
  return seoulRecordedAtISOFromYmd(year, month, day)
}
...
async function appendCraftPriceHistory(supabaseRest, crafts, source, options = {}) {
  if (!Array.isArray(crafts) || !crafts.length) return
  const recordedAt =
    options.recordedAt ||
    (options.historyDate
      ? recordedAtISOFromDateString(options.historyDate)
...
async function upsertCraftItems(supabaseRest, payload, options) {
  ...
  const recordedAt = await appendCraftPriceHistory(
    supabaseRest,
    enrichedCrafts,
    options && options.source ? options.source : 'admin',
    {
      historyDate: options && options.historyDate ? options.historyDate : undefined,
      recordedAt: options && options.recordedAt ? options.recordedAt : undefined
    }
  )

  const catalog = await readCraftsFromSupabase(supabaseRest)
  if (recordedAt) catalog.lastRecordedAt = recordedAt
  return catalog
}
...
const { verifyCraftAdmin: verifyAdminSecret } = require('./admin-auth')
...
module.exports = {
  ...
  getCraftsCatalog,
  getSeoulDailyRecordedAtISO,
  recordedAtISOFromDateString,
  seoulRecordedAtISOFromYmd,
